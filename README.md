...
一个 Chrome Manifest V3 浏览器扩展，用于在访问使用腾讯地图（TMap JS GL SDK）的第三方网站时，提供测距、多边形绘制和打点标记辅助工具。
...
| **绘制多边形** ⬡ | 点击地图打点绘制多边形，或直接输入坐标；支持多个多边形共存、选中、重命名、删除、顶点编辑、拖拽平移、坐标导入/导出 |
| **打点标记** 📍 | 点击地图标记命名点位；支持多点管理、重命名、显隐切换、Shift 多选、坐标导入/导出 |

**其他特性：**
- 点击面板列表中的多边形或点位，地图平滑定位（`panTo`）到该位置，缩放层级不变
- 多边形和点位为持久数据，切换工具后依然保留在地图上
- 面板可折叠为侧边小标签，不干扰正常浏览
- 面板可沿垂直轴拖拽自由定位（贴靠右侧）
- 设置面板提供调试日志开关
- 折叠/展开状态和位置持久化到 `localStorage`
...
│   │   ├── overlay-manager.ts  # 地图覆盖物管理（标记/线/标签/多边形/点位）
...
│           └── parse-coords.ts  # 坐标解析：parseCoords（多边形，≥3点）
│                                #            parsePointCoords（点位，≥1点）
...
构建产物在 `dist/` 目录：
- `hook.iife.js` — Hook 脚本
- `panel.iife.js` — Panel UI
- `manifest.json` — 扩展清单
- `icons/` — 扩展图标
...
4. 在 `src/hook/tool-manager.ts` 的 constructor 中注册新工具，并为各命令添加路由方法
5. 在 `src/hook/index.ts` 的消息 switch-case 中路由新 `PanelCmd`
6. 在 `src/panel/components/tools/` 中添加对应的结果展示组件
7. 在 `src/panel/composables/useTool.ts` 中处理新事件、维护响应式状态并暴露命令函数
