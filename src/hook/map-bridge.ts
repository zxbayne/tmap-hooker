...
/** prototype wrapper 内部用，一旦捕获过就设为 true，resetCapture() 重置它。 */
let captured = false

/**
 * 重置捕获状态。Panel 重连（SPA 切回地图页面）时调用，
 * 让下一次 TMap.Map 方法调用重新触发实例捕获。
 */
export function resetCapture(): void {
  captured = false
  log('resetCapture: capture flag cleared')
}
 ...
 * wrapper 永久保留（不再拆除），通过模块级 `captured` 标志控制是否触发捕获。
